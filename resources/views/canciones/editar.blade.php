...
@section('content')
    ...
    >
        ...
        <div class="col-12 mb-4">
            ...
            <input name="titulo" type="text" class="form-control" id="validationServer01" placeholder="{{$track->titulo}}" required>
        </div>
        <div class="input-group col-12 mb-4">
            <label class="input-group-text" for="inputGroupFile01">{{__('SUBE LA MUSICA')}}</label>
            <input id="music" type="file" name="music" class="form-control @error('music') is-invalid @enderror" id="inputGroupFile01">
        </div>
        <div class="col-12">
            <button class="btn btn-primary" type="submit">{{$boton}}</button>
            <a class="btn btn-danger" href="{{route('track.index')}}">{{__('CANCELAR')}}</a>
        </div>
    </form>
    @error('titulo')
